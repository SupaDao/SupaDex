...
/// @title TickMathOptimized
/// @notice Math library for computing sqrt prices from ticks and vice versa
/// @dev Adapted from Uniswap V3 TickMath but with 0.8.x overflow checks and potential gas optimizations
library TickMathOptimized {
    ...
    function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtPriceX96) {
        unchecked {
            ...
            if (absTick & 0x100 != 0) ratio = (ratio * 0xfcbe86c7900a88aedcffc5d932334448) >> 128;
    ...
    function getTickAtSqrtRatio(uint160 sqrtPriceX96) internal pure returns (int24 tick) {
        unchecked {
            ...
            // forge-lint: disable-next-line(unsafe-typecast)
            int256 logBase2 = (int256(msb) - 128) << 64;
            ...
            assembly {
                r := shr(127, mul(r, r))
                let f := shr(128, r)
                logBase2 := or(logBase2, shl(54, f))
                r := shr(f, r)
            }
            assembly {
                r := shr(127, mul(r, r))
                let f := shr(128, r)
                logBase2 := or(logBase2, shl(53, f))
                r := shr(f, r)
            }
            assembly {
                r := shr(127, mul(r, r))
                let f := shr(128, r)
                logBase2 := or(logBase2, shl(52, f))
                r := shr(f, r)
            }
            assembly {
                r := shr(127, mul(r, r))
                let f := shr(128, r)
                logBase2 := or(logBase2, shl(51, f))
                r := shr(f, r)
            }
            assembly {
                r := shr(127, mul(r, r))
                let f := shr(128, r)
                logBase2 := or(logBase2, shl(50, f))
            }

            int256 logSqrt10001 = logBase2 * 255738958999603826347141; // 128.128 number
